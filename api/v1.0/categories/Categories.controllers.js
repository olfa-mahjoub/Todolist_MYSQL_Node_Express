...
/**
 * Crée une nouvelle catégorie et met à jour le cache Redis.
 */
exports.create = async (req, res) => {
  try {
    const data = await sequelizeObject.create(req.body);

    // Met à jour le cache Redis avec toutes les catégories
    const allRecords = await sequelizeObject.findAll();
    await SET_ASYNC("categories", JSON.stringify(allRecords), "EX", 50); // Expire dans 50 secondes
...
/**
 * Récupère toutes les catégories, en utilisant Redis si disponible.
 */
exports.getAll = async (req, res) => {
  try {
    const cachedData = await GET_ASYNC("categories");
    ...
    await SET_ASYNC("categories", JSON.stringify(data), "EX", 50);
...
/**
 * Met à jour une catégorie par ID et rafraîchit le cache Redis.
 */
exports.update = async (req, res) => {
  ...
  try {
    ...
    if (updated) {
      ...
      const allRecords = await sequelizeObject.findAll();
      await SET_ASYNC("categories", JSON.stringify(allRecords), "EX", 50);
...
/**
 * Supprime une catégorie par ID et rafraîchit le cache Redis.
 */
